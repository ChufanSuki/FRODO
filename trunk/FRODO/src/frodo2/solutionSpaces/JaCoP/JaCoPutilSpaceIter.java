...
public class JaCoPutilSpaceIter < U extends Addable<U> > extends HypercubeIter<AddableInteger, U>{
	...
	public JaCoPutilSpaceIter(JaCoPutilSpace<U> space, String[] variables, AddableInteger[][] domains, AddableInteger[] assignment){
		...
		for (int i = 0; i < nbrVars; i++) {
			...
			if (Math.log((double) nbrSolLeft) + Math.log((double) domSize) >= Math.log(Long.MAX_VALUE)) 
				throw new OutOfMemoryError ("Too many solutions to fit in a long");
			nbrSolLeft *= domSize;
		}
	...
	protected int iter(){
		...
		this.utility = space.getUtility(myUtilIndex); /// @todo Why go through the utility index? Can't this be improved?
		this.nbrSolLeft--;
