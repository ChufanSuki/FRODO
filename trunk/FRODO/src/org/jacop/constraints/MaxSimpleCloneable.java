...
/** A cloneable version of the MaxSimple constraint
 * @author Thomas Leaute
 */
public class MaxSimpleCloneable extends MaxSimple implements ConstraintCloneableInterface<MaxSimpleCloneable> {
     ...
     */
	public MaxSimpleCloneable(IntVarCloneable x1, IntVarCloneable x2, IntVarCloneable max) {
		super(x1, x2, max);
	}

	/** @see org.jacop.constraints.ConstraintCloneableInterface#cloneInto(org.jacop.core.StoreCloneable) */
	@Override
	public MaxSimpleCloneable cloneInto(StoreCloneable targetStore) throws FailException {
		...
		IntVarCloneable maxClone = targetStore.findOrCloneInto((IntVarCloneable) this.max);
		if (maxClone.dom().isEmpty()) 
			throw StoreCloneable.failException;
		
		return new MaxSimpleCloneable (x1clone, x2clone, maxClone);
	}
